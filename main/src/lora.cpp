...
constexpr std::uint8_t TX_pin =
    15; // must rename because of conflict with one of libs
...
void init() {
    ...
    atCommand = String("AT+TEST=RFCFG, ") + frequency + ", " + spreadingFactor +
...
String gen_rand() {
    ...
    for (int i = 0; i < 17; i++) {
        ...
        result += ", "; // comma added to get needed format of string
    }
    return result;
}
