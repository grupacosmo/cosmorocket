...
BME280_INTF_RET_TYPE platform_write(uint8_t reg_addr, const uint8_t *reg_data,
    ...
    return g_i2c_device.write(reg_addr, reg_data, length)
...
BME280_INTF_RET_TYPE platform_read(uint8_t reg_addr, uint8_t *reg_data,
                                   uint32_t length, void *intf_ptr) {
    return g_i2c_device.read(reg_addr, reg_data, length)
        .transform([](Success) -> BME280_INTF_RET_TYPE { return 0; })
        .value_or(-1);
}
...
auto init() -> std::expected<Success, Error> {
    return I2CDevice::newDevice(CONFIG_BME280_I2C_ADDR)
        .and_then([](I2CDevice device) -> std::expected<Success, Error> {
            ...
            // Filter is of no use to us, because we need to know values at the
            // moment, even if they reach extremes and last only a short while
